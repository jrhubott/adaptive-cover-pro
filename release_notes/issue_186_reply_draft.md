...
**Force Override and Weather Override bypass Automatic Control by design.** When you configure a binary sensor (wind, rain, window-open, alarm) as a Force Override sensor, it _must_ move covers regardless of whether sun-tracking is paused — that is the whole point of the feature. Turning Automatic Control OFF does not silence those handlers; it silences only solar tracking, climate, cloud suppression, glare zones, and custom positions.
...
| `adaptive_cover_pro.emergency_stop`      | **Panic button.** Stops _every_ configured cover (not just in-flight ones), then disables. With no target, hits every ACP instance in your installation. |
