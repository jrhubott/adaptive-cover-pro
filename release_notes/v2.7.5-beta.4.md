# 🎯 Adaptive Cover Pro v2.7.5-beta.4
...
- **When ANY sensor detects motion** → Covers use automatic sun-based positioning for glare control
- **When ALL sensors show no motion for configured timeout** → Covers return to default position (energy savings/privacy)
...
- ✅ 22 new comprehensive tests for motion control:
...
1. **Add motion sensors to existing integration instance:**
   ...
   - Go to Settings → Devices & Services → Adaptive Cover Pro
   - Click Configure on your integration instance
   - Navigate to Automation settings
   - Add motion sensors (binary*sensor.motion*\*)
   - Set timeout duration (default 300 seconds works well)

2. **Test behavior:**

   - Trigger motion sensor → Covers should use automatic sun positioning
   - Wait for timeout after motion stops → Covers should return to default position
