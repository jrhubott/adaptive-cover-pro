...
| `sensor.{device_name}_sun_elevation` | Enabled | Current sun elevation angle in degrees. Use to debug elevation constraints and blind spots. Attributes: `valid_elevation` (bool); `min_elevation`/`max_elevation` when configured; `blind_spot_elevation` and `in_blind_spot` when a blind spot is enabled. |
