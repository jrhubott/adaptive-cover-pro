...
def test_restore_from_attributes_populates_manager_state():
    """A valid future expiry is restored into a live OverrideState."""
    ...
    restored = manager.override_for(eid)
    assert restored is not None
    expected_started_at = expiry - dt.timedelta(minutes=reset_minutes)
    actual_started_at = restored.started_at
...
def test_restore_from_attributes_drops_expired_entries():
    ...
    assert manager.is_cover_manual(eid) is False
    assert manager.override_for(eid) is None
...
def test_restore_from_attributes_filters_unknown_entities():
    ...
    assert manager.is_cover_manual(known) is True
    # Removed cover not restored
    assert manager.is_cover_manual(unknown) is False
    assert manager.override_for(unknown) is None
...
def test_restore_keeps_good_entries_alongside_a_corrupt_one():
    ...
    assert manager.is_cover_manual(good) is True
    assert manager.is_cover_manual(bad) is False
...
async def test_first_refresh_skips_apply_position_for_manual_cover():
    ...
    coordinator.manager.mark_user_command(eid_manual, reason="test setup")
...
def test_restore_preserves_started_at_math_with_timezone():
    ...
    restored = manager.override_for(eid)
    assert restored is not None
    restored_started_at = restored.started_at
