...
def test_config_flow_minor_version_reaches_highest_migration_target() -> None:
    ...
    Currently the highest target is 22 (the v3.21 → v3.22 no-op block that
    advances entries past the additive sunrise_gates_start flag, per issue
    #1340).
    Raise this assertion whenever a new minor migration block is added.
    """
    from custom_components.adaptive_cover_pro.config_flow import ConfigFlowHandler

    assert ConfigFlowHandler.MINOR_VERSION == 22
...
# ---------------------------------------------------------------------------
# v3.18 → v3.19 — additive solar-transmittance / solar-gain options
# (issues #1236 + #1237). Purely additive: absent keys read as "feature off".
# ---------------------------------------------------------------------------


async def test_migration_3_19_is_noop_and_idempotent(hass: HomeAssistant) -> None:
    """A minor-18 entry reaches 3.19 with options byte-identical, twice over.

    None of the new solar keys is seeded — an absent ``solar_properties_enabled``
    already reads as off, which is exactly what every existing install does. The
    block exists only so entries sitting at minor 18 stop re-triggering
...
async def test_migrate_v3_18_to_v3_19_preserves_configured_solar_options(
    hass: HomeAssistant,
) -> None:
    """An entry that already opted in keeps every value verbatim."""
    options = {
    ...
    entry = _make_entry(hass, dict(options), version=3, minor_version=18)
    assert await async_migrate_entry(hass, entry) is True
    assert entry.minor_version == 22
    for key, value in options.items():
...
# ---------------------------------------------------------------------------
# v3.19 → v3.20 — additive weather window-scope option (issue #1308)
# ---------------------------------------------------------------------------


async def test_migration_3_20_is_noop_and_idempotent(hass: HomeAssistant) -> None:
    """A minor-19 entry reaches 3.20 with options byte-identical, twice over.

    ``weather_outside_window`` is deliberately NOT seeded: an absent key
    already reads as "weather keeps acting outside the time window", which is
    what every install shipped before #1308 does. The block exists only so
    entries sitting at minor 19 stop re-triggering migration on every restart.
    """
    ...
    assert entry.minor_version == 22
    assert entry.options == before
    assert CONF_WEATHER_OUTSIDE_WINDOW not in entry.options
...
async def test_migrate_v3_19_to_v3_20_preserves_an_explicit_opt_out(
    hass: HomeAssistant,
) -> None:
    """An entry that already scoped weather keeps the stored False verbatim."""
    from custom_components.adaptive_cover_pro.const import CONF_WEATHER_OUTSIDE_WINDOW

    entry = _make_entry(
        hass,
        {CONF_WEATHER_ENABLED: True, CONF_WEATHER_OUTSIDE_WINDOW: False},
        version=3,
        minor_version=19,
    )
    assert await async_migrate_entry(hass, entry) is True
    assert entry.minor_version == 22
    assert entry.options[CONF_WEATHER_OUTSIDE_WINDOW] is False


# ---------------------------------------------------------------------------
# v3.20 -> v3.21 - additive per-slot scope-to-window flag (issue #1318)
# ---------------------------------------------------------------------------


async def test_migration_3_21_is_noop_and_idempotent(hass: HomeAssistant) -> None:
    """A minor-20 entry reaches 3.21 with options byte-identical, twice over.

    ``custom_position_scope_to_window_N`` is deliberately NOT seeded: an absent
    key already reads as "this slot keeps its exact position after the window
    closes", which is what every install shipped before #1318 does - and what
    #895 asked for explicitly. Seeding it either way would pick a side in a
    disagreement between two users; the block exists only so entries sitting at
    minor 20 stop re-triggering migration on every restart.
    """
    entry = _make_entry(
        hass,
        {
            "azimuth": 180,
            CONF_DEFAULT_HEIGHT: 60,
            "custom_position_sensors_1": ["binary_sensor.cloudy"],
    ...
    assert entry.options == before
    for n in range(1, 11):
        assert f"custom_position_scope_to_window_{n}" not in entry.options
...
async def test_migrate_v3_20_to_v3_21_preserves_an_opted_in_slot(
    hass: HomeAssistant,
) -> None:
    """An entry that already scoped a slot keeps the stored True verbatim."""
    entry = _make_entry(
        hass,
        {"custom_position_scope_to_window_2": True},
        version=3,
        minor_version=20,
    )
    assert await async_migrate_entry(hass, entry) is True
    assert entry.options["custom_position_scope_to_window_2"] is True
    assert entry.minor_version == 22


# ---------------------------------------------------------------------------
# v3.21 -> v3.22 - additive sunrise-gates-start opt-in (issue #1340)
# ---------------------------------------------------------------------------
...
async def test_migration_3_22_is_noop_and_idempotent(hass: HomeAssistant) -> None:
    """A minor-21 entry reaches 3.22 with options byte-identical, twice over.

    ``sunrise_gates_start`` is deliberately NOT seeded: an absent key reads as
    OFF, which is exactly what every install shipped before #1340 does - and
    #438 is the decision of record for that behaviour. Seeding it either way
    would pick a side between #438's reporter and #1340's; the block exists
    only so entries sitting at minor 21 stop re-triggering migration on every
    restart.
    """
    entry = _make_entry(
        hass,
    ...
    assert entry.minor_version == 22
    assert entry.options == before
    assert CONF_SUNRISE_GATES_START not in entry.options
...
async def test_migrate_v3_21_to_v3_22_preserves_an_explicit_opt_in(
    hass: HomeAssistant,
) -> None:
    """An entry that already opted in keeps the stored True verbatim."""
    entry = _make_entry(
        hass,
    ...
    assert await async_migrate_entry(hass, entry) is True
    assert entry.options[CONF_SUNRISE_GATES_START] is True
    assert entry.minor_version == 22


async def test_migrate_v1_cascades_to_minor_22(hass: HomeAssistant) -> None:
    """The oldest entry shape still lands on the current top minor in one pass."""
    entry = _make_entry(hass, {"azimuth": 180}, version=1, minor_version=1)
    assert await async_migrate_entry(hass, entry) is True
    assert entry.version == 3
    assert entry.minor_version == 22
