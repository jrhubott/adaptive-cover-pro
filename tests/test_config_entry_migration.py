...
def test_config_flow_minor_version_reaches_highest_migration_target() -> None:
    ...
    Currently the highest target is 21 (the v3.20 → v3.21 no-op block that
    advances entries past the additive per-slot custom_position_scope_to_window_N
    flag, per issue #1318).
    Raise this assertion whenever a new minor migration block is added.
    """
    from custom_components.adaptive_cover_pro.config_flow import ConfigFlowHandler

    assert ConfigFlowHandler.MINOR_VERSION == 21
...
async def test_migrate_v1_cascades_to_minor_21(hass: HomeAssistant) -> None:
