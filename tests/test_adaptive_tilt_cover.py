...
# fmt: off  # hand-laid so 810 golden rows read as a table, not 810 lines
...
class TestReflectedBeamFloor:
    ...
    def test_disabled_option_is_byte_identical_at_every_geometry(
        ...
        Both spellings of "disabled" are checked at every point — the field
        default (an install that never saw the option) and an explicit ``0``.
